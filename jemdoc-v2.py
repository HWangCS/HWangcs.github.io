...
class _UnreadableStream(object):
  """Wraps a stream to support unread (one char) for Python 3 compatibility."""
  def __init__(self, f):
    self._f = f
    self._putback = None
  def read(self, n=1):
    if self._putback is not None and n >= 1:
      c = self._putback
      self._putback = None
      if n == 1:
        return c
      return c + self._f.read(n - 1)
    return self._f.read(n)
  def readline(self):
    if self._putback is not None:
      c = self._putback
      self._putback = None
      if c == '\n':
        return c
      return c + self._f.readline()
...
def parseconf(cns):
  ...
  fs = [_UnreadableStream(StringIO(standardconf()))]
  for sname in cns:
...
def insertmenuitems(f, mname, current, prefix):
  m = _UnreadableStream(open(mname, 'r', encoding='utf-8', newline=None))
  cs = controlstruct(m)
  while pc(cs) != '':
    l = readnoncomment(m)
    l = l.strip()
...
def pc(f, ditchcomments=True):
  ...
  if c: # only undo forward movement if we're not at the end.
    if ditchcomments and c == '#':
      ...
      if doincludes(f, l):
        return "#"
    if isinstance(c, bytes) and c in b' \t':
      return pc(f)
    elif isinstance(c, str) and c in ' \t':
      return pc(f)

    if c == '\\' or c == b'\\':
      c += pc(f)

    if hasattr(f.inf, 'unread'):
      f.inf.unread(c)
    else:
      f.inf.seek(-1, 1)
  elif f.otherfiles:
  ...
  while pc(f) == char:
    ...
    if newlevel > level:
      for i in range(newlevel - level):
        if newlevel > 1:
          out(f.outf, '\n')
        if ul == 'ol' and level == 0 and i == 0 and 'publication' in (f.inname or ''):
          out(f.outf, '<ol reversed>\n<li>')
        else:
          out(f.outf, '<%s>\n<li>' % ul)
    elif newlevel < level:
      out(f.outf, '\n</li>')
...
def main():
  ...
  for inname in innames:
    ...
    infile = _UnreadableStream(open(inname, 'r', encoding='utf-8', newline=None))
